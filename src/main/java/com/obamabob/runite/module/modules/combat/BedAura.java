...
public class BedAura extends Module {
    ...
    Setting<String> attackMode = register(new Setting<>("Mode", this, "Own", new String[]{"Normal", "Own"}));
    Setting<Double> attackRange = register(new Setting<>("Attack Range",this,4.0,0.0,10.0));
    Setting<Integer> breakDelay = register(new Setting<>("Break Delay", this, 1,0,20));
    Setting<Integer> placeDelay = register(new Setting<>("Place Delay", this,1,0,20));
    Setting<Double> targetRange = register(new Setting<>("Target Range",this,7.0,0.0,16.0));
    Setting<Boolean> antiSuicide = register(new Setting<>("Anti Suicide", this, false));
    Setting<Integer> antiSuicideHealth = register(new Setting<>("Suicide Health", this, 14,1,36));
    Setting<Integer> minDamage = register(new Setting<>("Min Damage", this, 5,1,36));
    Setting<Boolean> rotate = register(new Setting<>("Rotate",this, true));
    Setting<Boolean> chatMsgs = register(new Setting<>("Chat Msgs", this, true));
    Setting<Boolean> disableNone = register(new Setting<>("Disable No Bed", this, false));
    ...
    private com.obamabob.runite.util.Timer breakTimer = new com.obamabob.runite.util.Timer();
    private com.obamabob.runite.util.Timer placeTimer = new com.obamabob.runite.util.Timer();
    ...
    public void onDisable() {
        ...
        if (chatMsgs.getValue()) {
            if (hasNone && disableNone.getValue()) {
                sendChatMessage("No beds detected... BedAura turned OFF!");
            }
        }
    ...
    private NonNullList<BlockPos> findTargetPlacePos(EntityPlayer entityPlayer) {
        ...
        targetPlacePos.addAll(getSphere(mc.player.getPosition(), attackRange.getValue().floatValue(), attackRange.getValue().intValue(), false, true, 0)
                .stream()
