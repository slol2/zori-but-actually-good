...
public class RenderUtil {
    ...
    public static void createColorPre(Color color, boolean isPlayer) {
        mc.getRenderManager().setRenderShadow(false);
        mc.getRenderManager().setRenderOutlines(false);
        GlStateManager.pushMatrix();
        glEnable(GL11.GL_POLYGON_OFFSET_FILL);
    ...
    public static void createWirePre(Color color, int lineWidth, boolean isPlayer) {
        mc.getRenderManager().setRenderShadow(false);
        mc.getRenderManager().setRenderOutlines(false);
        GlStateManager.pushMatrix();
    ...
    public static void drawBoxWithDirection(AxisAlignedBB bb, Color color, float rotation, double width, int mode) {
    ...
    public static void drawDirection(Points square, Color color, double width) {
    ...
    public static void drawLineWidth(double posx, double posy, double posz, double posx2, double posy2, double posz2, Color color, double width) {
        Tessellator tessellator = Tessellator.getInstance();
        BufferBuilder bufferbuilder = tessellator.getBuffer();
        GlStateManager.glLineWidth((float) width);
        GlStateManager.color(color.getRed()/255.0f,color.getGreen()/255.0f,color.getBlue()/255.0f,color.getAlpha()/255.0f);
    ...
    public static void drawNametag (Entity entity, String[] text, Color color, int type) {
        Vec3d pos = getInterpolatedPos(entity,mc.getRenderPartialTicks());
    ...
    public static void drawNametag (double x, double y, double z, String[] text, Color color, int type) {
        ...
        if (type == 2) {
            ...
            for (int i = 0; i < text.length; i++) {
                double w=mc.fontRenderer.getStringWidth(text[i])/2;
                if (w > width) {
                    width = w;
                }
            }
            drawBorderedRect(-width - 1, -mc.fontRenderer.FONT_HEIGHT, width + 2,1,1.8f, new Color(0,4,0,85), bcolor);
        }
        GlStateManager.enableTexture2D();
        for (int i=0;i<text.length;i++) {
            mc.fontRenderer.drawStringWithShadow(text[i],-mc.fontRenderer.getStringWidth(text[i])/2,i*(mc.fontRenderer.FONT_HEIGHT+1)+start,color.getRGB());
        }
    ...
    private static void drawBorderedRect(double x, double y, double x1, double y1, float lineWidth, Color inside, Color border) {
        Tessellator tessellator = Tessellator.getInstance();
        BufferBuilder bufferbuilder = tessellator.getBuffer();
        GlStateManager.color(inside.getRed()/255.0f,inside.getGreen()/255.0f,inside.getBlue()/255.0f,inside.getAlpha()/255.0f);
        bufferbuilder.begin(GL11.GL_QUADS, DefaultVertexFormats.POSITION);
        ...
        tessellator.draw();
        GlStateManager.color(border.getRed()/255.0f,border.getGreen()/255.0f,border.getBlue()/255.0f,border.getAlpha()/255.0f);
    ...
    public static void draw2DRect(int posX, int posY, int width, int height, int zHeight, Color color) {
        Tessellator tessellator = Tessellator.getInstance();
        BufferBuilder bufferbuilder = tessellator.getBuffer();
